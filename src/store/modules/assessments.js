...
const actions = {
  fetchAssessments ({ commit }) {
    ...
    axios.get(`${types.BASE_URL}/image-assessments?page[size]=12`, {
      headers: {
      ...
      .then((res) => {
        commit('fetchAssessments', res.data.data)
      })
  },
  ...
  fetchAssessmentsById ({ commit, state }, id) {
    return GETApi(`${types.BASE_URL}/image-assessments/${id}`).then(res => res)
  },
